...
//for tab one
  ...
  const handleTabOneChangeDriver = (selectedOption) => {
    ...
    const DSalary = Math.round((driversalary/30).toString())
    // Update state for tabone and save driversalary
    setTabone({ ...tabone, driverName: value });
    setDriverSalary(DSalary);
  }

  const options = driver.map(option => ({
    value: option.d_ID,
...
// Helper function to parse time strings like "HH:MM:SS" to milliseconds
...
useEffect(() => {
    if (tabtwo.exitDate && tabtwo.dcReturnDate && tabtwo.exitTime && tabtwo.dcReturnTime) {
        ...
        const exitDate = new Date(tabtwo.exitDate);
        const returnDate = new Date(tabtwo.dcReturnDate);

        const exitDateTime = new Date(exitDate.getTime() + parseTimeToMilliseconds(tabtwo.exitTime));
        const returnDateTime = new Date(returnDate.getTime() + parseTimeToMilliseconds(tabtwo.dcReturnTime));
        ...
        // console.log('Before setting returnHours:', tabtwo.returnHours);
        setTabtwo(prevTabtwo => {
            // console.log('Updating returnHours to:', formattedTime);
            return { ...prevTabtwo, returnHours: formattedTime };
        });
    }
}, [tabtwo.returnDays, tabtwo.dcReturnTime]);

// useEffect(() => {
//     // console.log('Component re-rendered with tabtwo:', tabtwo);
// }, [tabtwo]);
  ...
  const validateClkm = () => {
    if (parseFloat(tabthree.clkm) <= parseFloat(tabthree.opkm)) {
      alert("CL KMs must be greater than OP KMs.");
      setTabthree({ ...tabthree, clkm: '' });
    }
  };
  ...
  useEffect(() => {
    if (tabfour.dcngconsumption) {
      ...
      setTabfour(prevTabfour => ({
        ...
        actavgkm: calculatedactavgkm.toFixed(2)
      }));
    }
...
// useEffect(() => {
//     if(tabtwo.returnDays){
//         const LorryRent = (parseFloat(sessionStorage.getItem('lory_Rent'))/30).toFixed(2) || 0;
...
//             ...prevTabfive, 
//             loryrent: LorryRent.toString(),
//             loryrenttrip: loryRentperTrip.toString(),
//             mtdloryrent: MTDLoryRent.toFixed(2),
//             driversalary:driverSalary.toString(),
//             mtddriversalary: MTDDriverSalary.toFixed(2),
//             fixedcostdriversalary: FixedCostDriverSalary.toFixed(2),
//             gps: GPS.toFixed(2)
//         }));
//     }
// },[tabtwo.returnDays])
...
useEffect(() => {
    if(tabtwo.returnDays){
        ...
        const loryRentperTrip = LorryRent * RETHrs;
        const MTDLoryRent = loryRentperTrip + parseFloat(allMTD.mtdlorrywiserent);
        const FixedCostDriverSalary = driverSalary * RETHrs;
        const MTDDriverSalary = FixedCostDriverSalary + parseFloat(allMTD.mtddriversalary);
        const GPS = (200 / 30) * RETHrs;

        setTabfive(prevTabfive => ({
            ...prevTabfive, 
            loryrent: Math.round(LorryRent).toString(),
            loryrenttrip: Math.round(loryRentperTrip).toString(),
  ...
  }, [tabtwo.returnDays, driverSalary, allMTD]);
...
const handleTabSixChange = (e) => {
    ...
    // const alphanumericFields = ['exitno', 'bgttrdays'];
...
}, [tabfour.tot, tabseven.totalcost, tabseven.repmtn, tabseven.totaltaxcash, tabseven.totaltaxfastag, tabseven.rto, tabseven.mechanicalRec, tabseven.challan, tabseven.borderexp, tabseven.loadunload, tabseven.inc, tabseven.otherexp]);
...
const validateInputstabSeven = () => {
    ...
    if (!tabseven.repmtn) {
      return false;
        ...
        return false;
    }else if(!tabseven.conscost){
        return false;
    }else if(!tabseven.remark){
        return false;
...
useEffect(() => {
    if(tabfive.loryrenttrip && tabfive.fixedcostdriversalary && tabfive.gps && tabnine.routeexp){
       ...
       setTabnine(prevState => ({ ...prevState, actualcost: ActualCost.toString() }));
...
const handleFinalSubmit = async (val) => {
    ...
    try{
        ...
        const requestBody = {
            ...
            trip_ID: 0,
            vechile_ID: V_ID,
            driverid: (tabone.driverName).toString(),
            actionperformedby: sessionStorage.getItem('UserId'),
            vehiclE_NO: tabone.lorryNo,
            to: tabone.destination ? JSON.stringify(tabone.destination.split(',').length) : '',
            nO_OF_STR: tabone.destination,
            exiT_DT: tabtwo.exitDate ? formatDate(tabtwo.exitDate) : null,
            exiT_TIME: tabtwo.exitTime,
            dC_RET_DATE: tabtwo.dcReturnDate ? formatDate(tabtwo.dcReturnDate) : null,
            ...
            driveR_SALARY: JSON.stringify(driverSalary),
            ...
            reT_LOADING_DATE: tabeight.retloadingdate ? formatDate(tabeight.retloadingdate) : null,
            reT_LOADING_EXIT_DATE: tabeight.retloadingexitdate ? formatDate(tabeight.retloadingexitdate) : null,
            reT_LO_HALT_DAYS: tabeight.rethaultdays,
            reT_WT: tabeight.retwt,
            reT_PKD: tabeight.retpkd,
            reT_VAL: tabeight.retval,
            reT_REC_DATE: tabeight.retrecdate ? formatDate(tabeight.retrecdate) : null,
            reT_UNLOAD_DT: tabeight.retrecdate ? formatDate(tabeight.retunloaddate) : null,
    ...
    {showPopup && (
        ...
        <div className='dashboard-one'>
            <div className='left-dashboard-one d-flex align-items-center gap-2'>
                ...
                <h5 className='dashHead m-0'>Trip Details</h5>
            </div>
                    ...
                    <div className='row d-flex'>
                        ...
                        <div className='col-6'>
                            ...
                            <div className='row d-flex mb-2 align-items-center'>
                                ...
                                {/* <div className='col-8'>
                                    <select className="form-control" id="exampleFormControlSelect1" name="destination" value={tabone.destination} onChange={handleTabOneChange}>
                                        <option value="" >Select Destination</option>
                                        <option value='NOIDA'>NOIDA</option>
                                        <option value='KAPASHERA'>KAPASHERA</option>
                                        <option value='27-MHPL-V2'>27-MHPL-V2</option>
                                        <option value='DELHI VRC SERVICES'>DELHI VRC SERVICES</option>
                                        <option value='MALDA-V2'>MALDA-V2</option>
                                        <option value='PRAYAGRAJ-(V2)'>PRAYAGRAJ-(V2)</option>
                                    </select>
                                </div> */}
                        ...
                        <div className='col-6'>
                            ...
                            <div className='row d-flex mb-2 align-items-center'>
                                ...
                                <div className='col-8'>
                                    ...
                                    <Select 
                                        name="driverName"
                                        value={options.find(option => option.value === tabone.driverName) || null}
                                        onChange={handleTabOneChangeDriver}
                                        options={options}
                                        isClearable
                    ...
                    <div className='row d-flex'>
                        ...
                        <div className='col-6'>
                            ...
                            <div className='row d-flex mb-2 align-items-center'>
                                ...
                                <div className='col-8'>
                                    <input type='text' className='form-control' name="clkm" value={tabthree.clkm} onBlur={validateClkm} onChange={handleTabThreeChange}></input>
                                </div>
                            </div>
                ...
                <div className='detailsTrip'>
                    <div className='row d-flex'>
                        ...
                        <div className='col-6'>
                            ...
                            <div className='row d-flex mb-2 align-items-center'>
                                <div className='col-4'>
                                    <p className='vehicleDPara'>Driver Salary/Day<span className='mandatory'>*</span></p>
                                </div>
                                <div className='col-8'>
                                    <input type='text' className='form-control' value={tabfive.driversalary || ''} readOnly></input>
                                </div>
                            </div>
                        ...
                        <div className='col-6'>
                            ...
                            <div className='row d-flex mb-2 align-items-center'>
                                <div className='col-4'>
                                    <p className='vehicleDPara'>Lorry Rent/Day<span className='mandatory'>*</span></p>
                                </div>
                                <div className='col-8'>
